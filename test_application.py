...
class Test_more_route(unittest.TestCase):
    ...
    def test_more(self):
        ...
        result = requests.post(f"{default_url}/more", data=data).json()
    ...
    def test_home(self):
        #home route test
        result = requests.get(f"{default_url}")
        self.assertEqual(result.status_code, 200)

        result = requests.get(f"{default_url}/feeds")
        self.assertEqual(result.status_code, 200)

    def test_category(self):
        #category route test
        categories = ["home", "computing", "electronics", "health-and-beauty", "fashion", "home-and-office", "phones-and-tablets"]
        result = requests.get(f"{default_url}/category/{random.choice(categories)}")
        self.assertEqual(result.status_code, 200)
    ...
    def test_search(self):
        ...
        query = ["android", "iphone", "tecno", "infinix", "samsung"]
        categories = ["home", "computing", "electronics", "health-and-beauty", "fashion", "home-and-office", "phones-and-tablets"]
        result = requests.get(f"{default_url}/search/{random.choice(categories)}/20")
        self.assertEqual(result.status_code, 200)
    ...
    def test_search_query(self):
        ...
        result = requests.get(f"{default_url}/search", params=data)
        self.assertEqual(result.status_code, 200)
